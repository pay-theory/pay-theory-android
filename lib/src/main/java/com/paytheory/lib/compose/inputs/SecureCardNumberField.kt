...
fun SecureCardNumberField(
    ...
    var secureString by rememberSaveable(
        stateSaver = createSecureStringWrapperSaver(),
        key = "clearKey_$clearKey"
    ) {
    ...
    SecureBaseTextField(
        ...
        onValueChange = { newSecureString ->
            secureString = SecureStringWrapper(
                SecureString(newSecureString.secureValue.revealForUi().take(20)),
                selection = null
