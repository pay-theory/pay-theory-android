...
private const val INVALID_APIKEY = "Invalid api key"
private const val INVALID_SERVICE_FEE = "Invalid service fee"
...
private const val BANK_ACCOUNT = "ach"
 ...
 */
class PayTheoryFragment(
    private val apiKey: String,
    private val transactionType: TransactionType? = TransactionType.CARD,
    private val feeMode: String? = FeeMode.INTERCHANGE
) : Fragment() {
    ...
    var cardNumberValid: Boolean = false
    var cardExpirationValid: Boolean = false
    var cardCVVValid: Boolean = false
    var zipCodeValid: Boolean = false
    var bankAccountNumberValid: Boolean = false
    var bankRoutingNumberValid: Boolean = false
    var cashBuyerValid: Boolean = false
...
//        payTheoryTransaction = Transaction(this.requireActivity().applicationContext,apiKey, constants!!,
//            partner!!, stage!!
//        )
    }
...
//        if (isCardPayment) {
//            val ccNumberValidation: (PayTheoryEditText) -> CardNumberFormattingTextWatcher =
//                { pt -> CardNumberFormattingTextWatcher(this, pt)  }
//            val cvvNumberValidation: (PayTheoryEditText) -> CVVFormattingTextWatcher =
//                { pt -> CVVFormattingTextWatcher(this, pt) }
//            val expirationValidation: (PayTheoryEditText) -> ExpirationFormattingTextWatcher =
//                { pt -> ExpirationFormattingTextWatcher(this, pt) }
//            val zipCodeValidation: (PayTheoryEditText) -> ZipCodeFormattingTextWatcher =
//                { pt -> ZipCodeFormattingTextWatcher(this, pt) }
//
...
//                { pt -> RoutingNumberFormattingTextWatcher(this, pt)  }
//
...
//                { pt -> CashBuyerContactTextWatcher(this, pt) }
//
...
//    }
    ...
    fun areFieldsValid(): Boolean {
        ...
        if (transactionType == TransactionType.CARD){
            return cardNumberValid && cardCVVValid && cardExpirationValid && zipCodeValid
        }
        //check if bank fields are valid
        if (transactionType == TransactionType.BANK){
            return bankAccountNumberValid && bankRoutingNumberValid && zipCodeValid
        }
        //check if cash fields are valid
        if (transactionType == TransactionType.CASH){
            return cashBuyerValid
        }
    ...
    private fun validateInputParameters(){
        ...
        if (feeMode != FeeMode.INTERCHANGE && feeMode != FeeMode.SERVICE_FEE){
            throw IllegalArgumentException(INVALID_SERVICE_FEE)
        }

        val startIndex: Int = apiKey.indexOf('-')
        this.partner = apiKey.substring(0, startIndex)
        val endIndex = apiKey.indexOf('-', apiKey.indexOf('-') + 1)
        this.stage = apiKey.substring(startIndex + 1, endIndex)

        if (this.stage != PAYTHEORYLAB && this.stage != PAYTHEORYSTUDY && this.stage != PAYTHEORY) {
            throw IllegalArgumentException(INVALID_APIKEY)
        }
        this.constants = Constants(this.partner!!, this.stage!!)
    }
}
